							...
							<tr>
								...
								<c:choose>
									<c:when test="${k.active == 1 }">
										<td><span style="color: lightgray">삭제된 게시물 입니다.</span></td>
									</c:when>
									<c:otherwise>
					...
					<td colspan="4">
						<ol class="paging">
						    ...
						    <c:choose>
						    	...
						    	<c:otherwise>
						    		<li><a href="/day05?cPage=${paging.beginBlock - paging.pagePerBlock }">이전으로</a></li>
						    	</c:otherwise>
						    </c:choose>
						    <!-- 블록안에 들어간 페이지번호들 -->
						    ...
						    <!-- 현재블록 인지 아닌지 나누자  -->
						    	<c:choose>
						    		...
						    		<c:otherwise>
						    			<li><a href="/day05?cPage=${k}">${k}</a></li>
						    		</c:otherwise>
						    	</c:choose>
						    </c:forEach>
							<!-- 다음 -->
							<c:choose>
						    	...
						    	<c:otherwise>
						    		<li><a href="/day05?cPage=${paging.beginBlock + paging.pagePerBlock }">다음으로</a></li>
						    	</c:otherwise>
						    </c:choose>
						</ol>
					</td>
