...
public class BBSController {
	 ...
	 * bbsService.getBbsList()); mv.setViewName("day05/list");
	 * 
	...
	public ModelAndView getBbsList(HttpServletRequest request) {
		try {
			...
			//	페이징에 넣어주기 setTotalRecord
			paging.setTotalRecord(count);
			
			//2. 전체 페이지의 수 
			//	numPerPage=4 기준으로 계산 가능
			//	numPerPage=4보다 작으면 전체 페이지는 1페이지
			if(paging.getTotalRecord()<=paging.getNumPerPage()) {
				paging.setTotalPage(1);
			}else {
				//전체레코드를 numPerPage로 나눈 정수값이 전체 페이지 
			...
			//	list로 갈 때마다 현재 페이지값을 전달하는 파라미터
			String cPage = request.getParameter("cPage");
			//cPage 넘어오지 않으면 무조건 현재 페이지가 1page가 된다.
			if(cPage ==null) {
				paging.setNowPage(1);
			}else {
			...
			//4. DB에서 nowPage를 기준으로 필요한 게시물 계산하기 
			//MySQL, MariaDB 는 limit, offset
			//	offset = limit * (현재 페이지 - 1)
			//	limit => numPerpage 같다? 내장함수
	...
	public ModelAndView getBbsDeleteOk(BbsVO bvo, @ModelAttribute("b_idx") String b_idx, @ModelAttribute("cPage") String cPage) {
		try {
			ModelAndView mv =new ModelAndView();
	...
	public ModelAndView getCommDelete(CommVO cvo, @ModelAttribute("b_idx") String b_idx, @ModelAttribute("cPage") String cPage) {
		try {
			ModelAndView mv = new ModelAndView();
